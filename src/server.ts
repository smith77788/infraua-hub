...
async function savePoint(
  ...
  /*
   * Гріємо укриття навколо нової точки — мовчки й не чекаючи.
   *
   * Заміряно на живому Overpass: перший запит по клітинці займає від 1,4 до
   * 11,4 секунди. Одинадцять секунд після натискання «куди сховатися» під
   * тривогою — це марна кнопка: людина за цей час вирішить, що бот завис.
   * Але рівно ці секунди можна витратити ЗАРАЗ, коли вона нікуди не біжить.
   *
   * Без `await`: відповідь про збережену точку не має чекати на мережу, а
   * помилка тут нічого не ламає — кнопка просто відпрацює як раніше. На
   * довгоживучому процесі (де й працює бот) обіцянка встигає; на короткому
   * ізоляті — ні, і це теж нормально.
   */
  void import("./lib/infra.functions").then((m) => m.fetchShelters(lat, lon)).catch(() => {});
