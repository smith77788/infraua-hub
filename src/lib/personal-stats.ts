...
export function renderStats(summary: StatsSummary | null): string {
  ...
  if (summary.avgLeadMin !== null && summary.aheadCount > 0) {
    ...
    // Чесність лишається — випередити не вдалося, і ми цього не ховаємо, — але
    // без жала: не сухий докір, а спокійне «ми поруч і стараємось». Людині й
    // так важко; підсумок має підтримувати, а не додавати провини.
    lines.push(
      "⏱ Цього періоду сирена звучала раніше за нас — тримаємось поруч і стараємось випередити наступного разу.",
    );
  }
  ...
  lines.push("");
  // Тепле слово наостанок — щоб підсумок лишав по собі підтримку, а не тягар.
  lines.push("Бережіть себе — ми щоночі поруч. 🇺🇦");
  lines.push("");
  lines.push(
