...
export interface Subscriber {
  ...
  okAt?: number | null;
  /**
   * Коли над людиною востаннє був рівень «в укриття».
   *
   * Потрібне не для рішення, а для дзеркального питання після відбою: хто був
   * під загрозою й досі не відмітився. Без цієї позначки «перевірте рідних»
   * довелося б вгадувати.
   */
  lastDangerAt?: number | null;
  /** Коли коло востаннє сповістили ПРО цю людину (кулдаун за темою). */
  lastCircleAlertAt?: number | null;
  /**
   * Коли ця людина востаннє отримала тривогу ПРО когось із кола.
   *
   * Окремо від `lastCircleAlertAt`, бо це інша межа. Кулдаун за темою рахує
   * «не повторюймо про Олену»; ця позначка рахує «не завалюймо Олену
   * повідомленнями про всіх інших». У родинному колі, де всі в одному місті,
   * без неї кожен дістав би по повідомленню про кожного.
   */
...
/**
 * За скільки хвилин до ймовірного підльоту нічний фільтр типів перестає діяти.
 *
 * Чверть години — відстань близько 45 км на типовій швидкості поршневого
 * шахеда. Менше було б замало, щоб устигнути в укриття; більше перетворило б
 * нічний режим на денний.
 */
export const NIGHT_WAKE_MIN = 15;
...
export function decideAlert(
  ...
  if (night) {
    if (sub.night === "silent") {
      // Повна тиша — явний вибір людини, і його не пробиває ніщо.
      return { send: false, reason: "нічний режим: тиша", ids: [], level };
    }
    /*
     * Нічний фільтр типів не діє, коли ціль ось-ось прилетить.
     *
     * ЩО БУЛО. Заміряно на симуляції двогодинного нальоту шести шахедів, що
     * заходять на точку людини: з УСТАЛЕНИМИ налаштуваннями вночі виходило
     * **нуль** сповіщень, удень — сім. Усталене `night: "critical"`, а шахед не
     * входить у критичні типи. Тобто типова конфігурація мовчала рівно в тій
     * події, заради якої продукт існує, — нічний наліт шахедів.
     *
     * ЧОМУ НЕ ЗА РІВНЕМ. Перша версія винятку пропускала рівень «в укриття» —
     * і виявилась надто широкою. Заміряно: БЕЗ заміряної швидкості рівень «в
     * укриття» має КОЖЕН шахед у радіусі ста кілометрів, бо нижній край вилки
     * бере 600 км/год (реактивна «Герань»). Рівень у цьому випадку не розрізняє
     * нічого, тож виняток за ним скасував би нічний режим цілком.
     *
     * ЧОМУ ЗА ЙМОВІРНИМ ЧАСОМ. `minutesToNearest` — найімовірніша оцінка, і
     * вона розрізняє: 10 хв на 30 км, 13 на 40, 19 на 60, 32 на 100. Поріг у
     * чверть години — це «ціль ось-ось», а не «ціль десь є».
     *
     * Асиметрія «поспішити безпечно» тут не порушена: вона керує РІВНЕМ і
     * текстом сповіщення, які читають найшвидший край, і це лишається як було.
     * Тут вирішується інше питання — чи будити людину, яка просила вночі тиші
     * для дронів, — і на нього чесно відповідає саме ймовірний час.
     *
     * Обіцянка «нічний режим лише звужує денний» ціла: `tier` звужує в обох
     * режимах і тут не чіпається, тож хто просив лише ракети, лише ракети й
     * отримає. Вночі пробиває не ТИП, а близькість.
     */
    const soon = assess.minutesToNearest !== null && assess.minutesToNearest <= NIGHT_WAKE_MIN;
    if (sub.night === "critical" && !soon) {
      pool = pool.filter((n) => critical.includes(n));
    }
  }
  if (pool.length === 0) {
    return {
