...
/**
 * Відбиток картини: набір цілей із огрубленою до ~1 км позицією + розмір групи.
 * Однаковий відбиток означає «нічого суттєвого не змінилось» — такий кадр не
 * пишемо, щоб буфер не роздувався від тремтіння координат між опитуваннями фіду.
 *
 * Розмір групи (`count`) входить у відбиток навмисно: рій, що завис над одним
 * вузлом і зростає з 3 до 10, стоїть на місці за координатою — без count реплей
 * (чия мета — показати РОЗВИТОК нальоту) застиг би на «3» усю хвилю.
 */
export function fingerprint(threats: readonly Threat[]): string {
  return threats
    .map((t) => `${t.id}|${t.lat.toFixed(2)}|${t.lon.toFixed(2)}|${t.type ?? "?"}|${t.count ?? 1}`)
    .sort()
    .join(";");
}
