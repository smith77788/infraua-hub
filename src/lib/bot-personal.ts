...
export function renderSettings(sub: Subscriber): string {
  return [
    ...
    `Жива картка: <b>${sub.liveCardMode ? "увімкнена" : "вимкнена"}</b> (/картка)`,
    "",
    ...
    "<i>Жива картка збирає оновлення нальоту в одне повідомлення замість потоку. Ціна — правка не дає сповіщення: про нові цілі ви не почуєте, поки самі не відкриєте чат. Загострення й перше сповіщення тривоги лишаються гучними.</i>",
  ].join("\n");
}
