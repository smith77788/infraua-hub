...
export interface Facility {
  ...
  detail?: string;
  /**
   * Посилання на запис у джерелі — те, що можна відкрити й перевірити очима.
   * Порожнє для опорних записів: у них перевіряти нічого.
   */
  source: string;
  /**
   * Найвища напруга обʼєкта у вольтах, з тега OSM.
   *
   * Досі це число існувало лише як текст в описі, тож модель не могла ним
   * скористатися: підстанція 750 кВ і підстанція 110 кВ були для неї
   * однаковими вузлами, хоча в мережі це різні обʼєкти.
   */
  voltage?: number;
  /** Електрична потужність станції в МВт, з `plant:output:electricity`. */
  capacityMw?: number;
  /**
   * Звідки взявся обʼєкт.
   *
   * `osm` — запис в OpenStreetMap з власним id; `baseline` — опорний перелік,
   * вписаний руками з приблизними координатами, щоб консоль не була порожньою,
   * коли Overpass мовчить.
   *
   * Поле зʼявилося, бо опорні записи видавали себе за перевірені: їм
   * підставлялося посилання виду `openstreetmap.org/#map=14/lat/lon` — це вид
   * карти, а не запис. Кнопка «Джерело» відкривала сторінку, яка виглядає як
   * підтвердження, але не підтверджує нічого.
   */
  origin?: "osm" | "baseline";
}
